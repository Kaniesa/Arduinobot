...
namespace arduinobot_controller{
    ...
    CallbackReturn ArduinobotInterface::on_init(const hardware_interface::HardwareInfo & hardware_info){
        ...
        try{
            ...
            RCLCPP_FATAL_STREAM(rclcpp::get_logger("ArduinobotInterface"), "No Serial Port provided! Aborting" );
    ...
    CallbackReturn ArduinobotInterface::on_deactivate(const rclcpp_lifecycle::State & previous_state){
        ...
        if( arduino_.IsOpen()){
            ...
            try{
                ...
                RCLCPP_FATAL_STREAM(rclcpp::get_logger("ArduinobotInterface"), "Something went wrong while closing the connection the port " << port_ );
                return CallbackReturn::FAILURE;
            }
        }
    ...
    hardware_interface::return_type ArduinobotInterface::read(const rclcpp::Time & time, const rclcpp::Duration & period){
        ...
        position_states_ = position_commands_; //NOTE: this assumes that bot servo motors are perfect
    ...
    hardware_interface::return_type ArduinobotInterface::write(const rclcpp::Time & time, const rclcpp::Duration & period){
        ...
        try{
            ...
            RCLCPP_FATAL_STREAM(rclcpp::get_logger("ArduinobotInterface"), "Something went wrong while sending the message " << msg);
