    ...
    class TaskServer : public rclcpp::Node{
        ...
        private:
            ...
            void execute(const std::shared_ptr<rclcpp_action::ServerGoalHandle<arduinobot_msgs::action::ArduinobotTask>> goal_handle){
                ...
                {
                    arm_joint_goal = {0.0, 0.27, 0.0, 0.0, 0.0};
                    gripper_joint_goal = {0.18, 0.18};
                }
                else if( goal_handle->get_goal()->task_number == 1 )
                {
                    arm_joint_goal = {2.5, 1.3, 0.7, 0.7, 0.6};
                    gripper_joint_goal = {1.0};
                }
                // else if( goal_handle->get_goal()->task_number == 1 )
                // {
                //     arm_joint_goal = {2.5, 1.3, 0.7, 0.7, 0.6};
                ...
                {
                    RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Planner Suceed, moving the arm and gripper");
                    arm_move_group.move();
                ...
                RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Goal Suceeded");
            }
